...
test = 'https://look.163.com/live?id=387122108'
# test = 'https://look.163.com/live?id=272537171'

# test = 'https://look.163.com/live?id=87925413'
robot =  'VectorBob'
'https://iplay.163.com/live?id=318778782'
web_path = test
...
# yh
# https://look.163.com/live?id=265304236
# mz
# https://look.163.com/live?id=385819363
# Dr
# https://look.163.com/live?id=87925413
# ll
# 'https://look.163.com/live?id=82453683'
...
# def requestPlayerMove(player,guessed, move, ear):
#     while True: # we're going to keep asking the player for a move until they give a valid one
#         time.sleep(0.5) # added so that any feedback is printed out before the next prompt
 
#         # player.getMove(obscurePhrase(num_str, guessed), guessed)
#         move = move.upper() # convert whatever the player entered to UPPERCASE
#         if move == 'EXIT' or move == 'PASS':
#             return move
#         elif len(move) == 1: # they guessed a number
#             if move not in NUM: # the user entered an invalid letter (such as @, #, or $)
#                 # send text
#                 send_txt('not a Number. Try again.{}'.format(ear),1)
#                 continue
#             elif move in guessed: # this letter has already been guessed
#                 # send text
#                 send_txt('{} already guessed {}'.format(move, ear),1)
#                 continue
#             else:
#                 return move
...
def tushare_(code:str):
    try:
        data=ts.get_k_data(code)
        df_data = pd.DataFrame(data)
        today_data = df_data.iloc[-1]
        # print(today_data)
        final_str = '{}: 开:{} 关:{} 高:{} 低:{} 总:{}'.format(today_data.code,
                                                        today_data.open, 
                                                        today_data.close,
                                                        today_data.high,
                                                        today_data.low,
...
# print(tushare_('300274'))
opt = Options()
opt.add_experimental_option('debuggerAddress','localhost:8989')
# chrome.exe --remote-debugging-port=8989 --user-data-dir="C:\Jomaclass\Look\Look-master\chromeprofile2"
driver = webdriver.Chrome(executable_path = 'C:/Jomaclass/Look/Look-master/chromedriver.exe' ,chrome_options = opt)
...
# username.send_keys("13186015783")
# username.send_keys("13410281508")

# password = driver.find_element_by_xpath("//*[@id='j-portal']/div/div/div[2]/div/div[1]/div/div[2]/div/div/input")
# password.send_keys("123456abcD")
# password.send_keys("zxcvbnm098765")
# checkbox = driver.find_element_by_xpath("//*[@id='j-portal']/div/div/div[2]/div/div[1]/div/p/label/span").click()
...
for txt in User_Manual:
     ...
     send_txt(txt, 1)
     for _ in range(20):
        driver.find_element_by_xpath("//*[@id='CONTENT_ID']/div/div[2]/div[2]/div/div/div[2]/div[2]").click()
     time.sleep(2)
        ...
        try:
            ...
            try:
               ele = driver.find_element_by_xpath(
                "//*[@id='CONTENT_ID']/div/div[2]/div[2]/div/div/div[1]/div[{}]/span[3]".format(i))
               print(ele.getCssValue("color"))
                # num = driver.find_element_by_xpath(
                #     "//*[@id='CONTENT_ID']/div/div[2]/div[2]/div/div/div[1]/div[{}]/span[4]".format(i)).text
                # print('this is num' + num)
            except:
                print('no color be getten')
            if content[-1] != txt:
                content.append(txt)
                print(txt)
                print(content)
                ...
                try:
                    ...
                    if guest != robot:
                        ...
                        if guest in bl:
                            ...
                            send_txt('bist du blind oder taub! Schwachkopf:) {}'.format(guest), 1)
                        ...
                        if ('送了' in txt) and (('云贝' not in txt) and ('粉团之心' not in txt)) :
                        ...
                        if (txt == '@{}'.format(robot)) and (guest not in [owner, 'lukas_genie']):
                            txt_l = [
                                "Hello",
                                "problem?",
                                "What up y'all!"
                        ...
                        if txt.split(' ')[0] == '@{}'.format(robot):
                            if guest in [owner, 'vectortools']:
                                if txt.split(' ')[1].lower() == 'chat':
                                        ...
                                        try:
                                            ...
                                            if guess_game[-1] != txt:
                                                ...
                                                try:
                                                    ...
                                                    print(txt)
                                                    print(num)
                                                    if (txt.split(' ')[1] == 'exit') and (guest == owner):
                                            ...
                                            if guess_game[-1] != txt:
                                                ...
                                                try:
                                                    ...
                                                    if len(players_name)<number_of_players:
                                                        if (txt.split(' ')[0] == '@{}'.format(robot)) and (guest not in players_name):
                                                        ...
                                                        if txt.split(' ')[0] == '@{}'.format(robot) and txt.split(' ')[1].lower() == 'pass' and (guest == player.name):
                                                            send_txt('{} 弃权'.format(player.name),1)
                                                            playerIndex = (playerIndex + 1) % len(players)
                                                            player = players[playerIndex]
                                                            send_txt(showBoard(obscurePhrase(num_str, guessed), guessed),1)
                                                            pass
                                                        if txt.split(' ')[0] == '@{}'.format(robot) and txt.split(' ')[1] == 'exit' and (guest in [owner, '一个外号叫姥姥的张飞'] ):
                                                            send_txt('Exiting...',1)
                                                            break

                                                        if txt.split(' ')[0] == '@{}'.format(robot) and isinstance(int(txt.split(' ')[1]), int) and (guest == player.name):
                        ...
                        if ("晚安" in txt) and (guest != robot):
                            send_txt('Good Night ☽ !', 1)
                        if ("good night" in txt) and (guest != robot):
                            send_txt('Good Night ☽ !', 1)
                        if ("Good night" in txt) and (guest != robot):
                            send_txt('Good Night ☽ !', 1)
                        ...
                        if (txt.split(' ')[0] == '@{}'.format(robot)) and (guest in [owner]):
                            txt_l = [
                        ...
                        if (txt.split(' ')[0] == '@{}'.format(robot)) and (txt.split(' ')[1] == 'chat') and (guest in [owner, '一个外号叫姥姥的张飞']):
                            for i in range(chat):
                                print(i)
                                if i == (chat - 1):
                                    time.sleep(1)
                        ...
                        if txt == '进入了直播间':
                            ...
                            if (TextBlob(txt).detect_language() == 'ja') or (TextBlob(txt).detect_language() == 'ko') or (TextBlob(txt).detect_language() == 'de') or (TextBlob(txt).detect_language() == 'es') or (TextBlob(txt).detect_language() == 'it'):
