class PersonNameParser {
    ...
    constructor() {
        this.coperativeNameParts = /(^|\s)(GmbH|Ltd\.|Limited|AG|AB|Inc\.|Incorporated|Co\.|Corp\.|KG|plc|International|Group|grupo|Gruppe|Industries|Tbk|Institut|Institute|KGaA|Cie)/gi;
    }
    ...
    parse(input) {
        ...
        for (var p in persons) {
            var person = persons[p].trim();
            if (person.indexOf("SPIEGEL ONLINE, ") != -1) {
                persons[p] = "SPIEGEL ONLINE";
            }
            else if (this.coperativeNameParts.test(person)) {
                persons[p] = person;
            }
            else {
                var parsed = this.parseFullName(person);
                persons[p] = `${parsed.last}, ${parsed.first} ${parsed.middle} ${parsed.title}`;
            }
        }
        return persons;
    }
